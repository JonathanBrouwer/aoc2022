...
pub fn part1(inp: &str) -> u32 {
    parse_input(inp)
        ...
        .map(|(bid, b)| {
            ...
            for i in 0..24 {
                ...
                for state in states {
                    ...
                    if state[0].0 >= b.obsidian_robot_cost.0
                        ...
                        new_states.insert(State([
                            (upd_state[0].0 - b.obsidian_robot_cost.0, upd_state[0].1 + 1),
                    ...
                    if state[0].0 >= b.geode_robot_cost.0 && state[2].0 >= b.geode_robot_cost.1 {
                        new_states.insert(State([
                            (upd_state[0].0 - b.geode_robot_cost.0, upd_state[0].1 + 1),
                            upd_state[1],
            ...
            (bid + 1) as u32 * states.into_iter().map(|s| s.0[3].0).max().unwrap()
        })
        .sum()
}
